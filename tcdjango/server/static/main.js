 ...
 * @return {Promise<Array<Terracotta.IKey>>}
 */
function getTags(remote_host) {
    const tagUrl = `${remote_host}/tags/`;
    return httpGet(tagUrl).then((response) => response || []);
}
 ...
 * @return {Promise<Array<Terracotta.IKey>>}
 */
function getCollections(remote_host) {
    const collectionUrl = `${remote_host}/collections/`;
    return httpGet(collectionUrl).then((response) => response.results || []);
}

/**
 * @param {string} remote_host
 * @param {Array} query_params Key/val pairs of query parameters.
 * @param {number} limit Items per page
 * @param {number} page Page number
 *
 * @return {string} dataset URL.
 */
function assembleDatasetURL(remote_host, query_params, limit, page) {
    if (page > 0) {
        request_url = `${remote_host}/datasets/?limit=${limit}&page=${page}`;
    } else {
        request_url = `${remote_host}/datasets/?limit=${limit}`;
    }

    for (let i = 0; i < query_params.length; i++) {
        request_url += `&${query_params[i].key}=${query_params[i].value}`;
    }
    return request_url;
}
 ...
 */
function assembleMetadataURL(remote_host, ds_id) {
 ...
 */
function assembleSinglebandURL(remote_host, ds_id, options, preview) {

    let request_url;

    if (preview) {
        request_url = `${remote_host}/singleband/${ds_id}/preview.png?tile_size=${JSON.stringify(THUMBNAIL_SIZE)}`;
    } else {
        request_url = `${remote_host}/singleband/${ds_id}/{z}/{x}/{y}.png`;
    }
    ...
    updateComputedUrl(request_url, ds_id);
    return request_url;
}
...
function assembleColormapUrl(remote_host, colormap, num_values) {
    return `${remote_host}/colormap?colormap=${colormap}&stretch_min=0&stretch_max=1&num_values=${num_values}`;
}
 ...
 */
function initUI(remote_host, collections, tags) {
    ...
    let collectionSelect = document.getElementById('collection-selector');
    collectionSelect.innerHTML = '';
    for (let i = 0; i < collections.length; i++) {
        const collectionOption = document.createElement('option');
        collectionOption.innerHTML = collections[i].name;
        collectionOption.value = collections[i].slug;
        collectionSelect.addEventListener('change', searchFieldChanged);
        collectionSelect.appendChild(collectionOption);
    }

    let tagSelect = document.getElementById('tag-selector');
    tagSelect.innerHTML = '';
    for (let i = 0; i < tags.length; i++) {
        const tagOption = document.createElement('option');
        tagOption.innerHTML = tags[i].name;
        tagOption.value = tags[i].name;
        tagSelect.addEventListener('change', searchFieldChanged);
...
function storeMetadata(metadata) {
    ...
    STATE.dataset_metadata[metadata.id] = metadata;
}
 ...
 */
function updateSearchResults(remote_host = STATE.remote_host, collections = STATE.collections, tags = STATE.tags) {
    // initialize table header for search results
    ...
    selectedCollection = collectionSelector.options[collectionSelector.selectedIndex].value
    query_params.push({ key: 'collection', value: selectedCollection });
    // for (let i = 0; i < datasetSearchFields.length; i++) {
    //     const ds_field = datasetSearchFields[i];
    //     if (ds_field.value !== '') {
    //         query_params.push({ key: 'collection', value: ds_field.value });
    //     }
    // }
    const tagSelector = document.getElementById('tag-selector');

    for (let i = 0; i < tagSelector.options.length; i++) {
        opt = tagSelector.options[i];
        if (opt.selected) {
            query_params.push({ key: 'tag', value: opt.value});
        }
    }
    ...
    return httpGet(datasetURL).then((res) => {
        updateDatasetList(remote_host, res.results, collections, tags);
    });
}
 ...
 * @param {Array<Terracotta.IKey>} collections
 * @param {Array<Terracotta.IKey>} tags
 */
function updateDatasetList(remote_host = STATE.remote_host, datasets, collections, tags) {
    let datasetTable = document.getElementById('search-results');
    ...
    if (datasets.length === 0) {
        ...
        resultRow.innerHTML = `<td colspan=${tags.length}>No datasets found</td>`;
        datasetTable.appendChild(resultRow);
        return;
    }
    STATE.dataset_list = datasets;
    for (let i = 0; i < datasets.length; i++) {
        let currentDataset = datasets[i];
        let resultRow = document.createElement('tr');

        let resultEntry = document.createElement('td');
        resultEntry.innerHTML = currentDataset.name;
        resultRow.appendChild(resultEntry);

        resultRow.id = `dataset-${currentDataset.id}`;
        if (STATE.activeSinglebandLayer && STATE.activeSinglebandLayer.id == currentDataset.id) {
            resultRow.classList.add('active');
        }
        resultRow.classList.add('clickable');
        resultRow.addEventListener('click', toggleSinglebandMapLayer.bind(null, currentDataset.id));
        ...
        let thumbnailUrl = assembleSinglebandURL(remote_host, currentDataset.id, null, true);
        detailsRow.innerHTML = `<td colspan=${1}><img src=${thumbnailUrl} class='thumbnail-image'></td>`;
        resultRow.appendChild(detailsRow);

        datasetTable.appendChild(resultRow);

        // retrieve metadata
        httpGet(assembleMetadataURL(remote_host, currentDataset.id)).then((metadata) => storeMetadata(metadata));
    }
}
...
function updateColormap() {
    ...
    const ds_id = STATE.activeSinglebandLayer.id;
    updateSinglebandLayer(ds_id, false);
}
 ...
 */
function toggleSinglebandMapLayer(ds_id, resetView = true) {
    let currentKeys;
    ...
    if (!ds_id || compareArray(currentKeys, ds_id)) {
        return;
    }

    const layer_id = ds_id
    const metadata = STATE.dataset_metadata[layer_id];
    /**
     * @type {noUiSlider.SliderElement}
     */
    ...
    updateSinglebandLayer(ds_id, resetView);
}
 ...
 */
function updateSinglebandLayer(ds_id, resetView = true) {
    removeRasterLayer();
    ...
    if (STATE.current_singleband_stretch) {
        layer_options.stretch_min = JSON.stringify(STATE.current_singleband_stretch[0]);
        layer_options.stretch_max = JSON.stringify(STATE.current_singleband_stretch[1]);
    }
    const layer_url = assembleSinglebandURL(STATE.remote_host, ds_id, layer_options);
...
function rgbSearchFieldChanged() {
    ...
    const datasets = STATE.dataset_list;
    ...
    // });
    populateRgbPickers(remote_host, datasets);
}
 ...
 */
function populateRgbPickers(remote_host, rgbDatasets) {
 ...
 */
function updateComputedUrl(url, ds_id = null){
    ...
    let metadata = null;
    if (ds_id != null) {
        metadata = STATE.dataset_metadata[ds_id];
    }
...
function updateSinglebandStretch(reloadLayer = true){
    ...
    if (reloadLayer && STATE.activeSinglebandLayer) {
        const current_id = STATE.activeSinglebandLayer.id;
        updateSinglebandLayer(current_id, false);
    }
}
...
function initializeApp(hostname) {
    ...
    getColormapValues(hostname)
        .then(() => getTags(hostname))
        .then((tags) => {
            STATE.tags = tags;
            getCollections(hostname).then((collections) => {
                STATE.collections = collections;
                initUI(hostname, collections, tags);
                updateSearchResults();
